...
def show_history():
    ...
    history_listbox = tkinter.Listbox(history_window, font=("Arial", 14))
    history_listbox.pack(fill=tkinter.BOTH, expand=True)
    
    for entry in history:
        history_listbox.insert(tkinter.END, entry)
    
    def clear_history():
        history.clear()
        history_listbox.delete(0, tkinter.END)
    
    clear_button = tkinter.Button(history_window, text="Clear History", command=clear_history)
...
def button_clicked(value):
    global right_symbols, top_symbols, label, A, B, operator, history
    ...
    if value in right_symbols:
        if value == "=":
            if A is not None and operator is not None:
                ...
                history.append(f"{A} {operator} {B} = {remove_zero_decimal(result)}")
                clear_all()
